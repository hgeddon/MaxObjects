...
namespace c74 { namespace min { namespace lib {
	...
	public:
		...
		limiter(int a_channel_count = 2, int a_buffer_size = 512, number a_samplerate = 48000.0) {
			...
			m_meter_levels.resize(m_channelcount);
			m_meter_rms.resize(m_channelcount);
		}
		...
		sample_vector meters() {
			long channel;
			double temp;

			for (channel = 0; channel < m_channelcount; channel++) {
				temp = sqrt(m_meter_levels[channel] / (double)m_frame_count);
				if (temp < m_meter_rms[channel]) {
					// decay
					m_meter_rms[channel] = (m_meter_rms[channel] * m_decay_coeff) + (temp * m_decay_coeff_inv);
				}
				else {
					m_meter_rms[channel] = temp;
				}
			}

			// if current m_meter_rms < prev m_meter_rms decay (c * 0.9) + (p * 0.1)
			// if current m_meter_rms > prev m_meter_rms attack p = c
...
#pragma mark attributes
		...
		void metermode(bool a_state) {
			m_meter_mode = a_state;
		}
		number metermode() {
			return m_meter_mode;
		}
		void decaycoeff(double a_coeff) {
			m_decay_coeff = a_coeff;
			m_decay_coeff_inv = 1 - a_coeff;
		}
		number decaycoeff() {
			return m_decay_coeff;
		}
		...
		void lookahead(int sample_count_for_lookahead_buffer) {
			if (sample_count_for_lookahead_buffer < m_buffer_size) {
...
#pragma mark methods
		...
		void clear() {
			...
			std::fill(m_meter_levels.begin(), m_meter_levels.end(), 0.0); // set meters to 0.
			m_last = 1.0;
			m_lookahead_index = 0;
...
#pragma mark audio
        ...
        void operator()(c74::min::audio_bundle input, c74::min::audio_bundle output) {
			m_frame_count = input.frame_count();
			...
			for (auto i = 0; i < m_frame_count; ++i) {
				...
				for (auto channel = 0; channel < m_channelcount; ++channel) {
					...
					// Get meter levels before limiting
					if (m_meter_mode == false) {
						if (i == 0) {
							m_meter_levels[channel] = 0;
						}
						samp = x[i];
						// rectify
						if (samp < 0) {
						...
						m_meter_levels[channel] += samp * samp;
					}
				...
				m_gain_reduction = m_gain_buffer[lookahead_playback]; // snapshot gain reduction gr
	...
	private:
		...
		number								m_gain_reduction;	// gain reduction level for gr meter
		sample_vector						m_meter_levels;		// vector of levels for meters (abs and smoothed / rms / lufs)
		sample_vector						m_meter_rms;	// rms vector
		number								m_decay_coeff { 0.9 }; // normalised decay coeff
		number								m_decay_coeff_inv { 0.1 };
		bool								m_meter_mode { true }; // false = pre limit, true = post limit
		int									m_frame_count {0}; // number of samples to integrate
	};
