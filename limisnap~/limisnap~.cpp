...
typedef struct _limisnap {
	...
	double					x_release;
	char					x_meter_mode;
	double					x_meter_decay;
	t_atom					*x_meters;		// meter value array
...
t_max_err limisnap_setmetermode(t_limisnap* x, void* attr, long argc, t_atom* argv);
t_max_err limisnap_setmeterdecay(t_limisnap* x, void* attr, long argc, t_atom* argv);
...
void ext_main(void *r)
{
	...
	CLASS_ATTR_CHAR(c, "metermode", 0, t_limisnap, x_meter_mode);
	CLASS_ATTR_LABEL(c, "metermode", 0, "metermode");
	CLASS_ATTR_ACCESSORS(c, "metermode", NULL, limisnap_setmetermode);
	CLASS_ATTR_STYLE(c, "metermode", 0, "onoff");

	CLASS_ATTR_DOUBLE(c, "meterdecay", 0, t_limisnap, x_meter_decay);
	CLASS_ATTR_LABEL(c, "meterdecay", 0, "meterdecay");
	CLASS_ATTR_ACCESSORS(c, "meterdecay", NULL, limisnap_setmeterdecay);
	CLASS_ATTR_FILTER_CLIP(c, "meterdecay", 0.0, 1.0);
	...
	CLASS_ATTR_FILTER_MIN(c, "lookahead", 1);
	//CLASS_ATTR_FILTER_CLIP(c, "lookahead", 1, 256);
	...
	//CLASS_ATTR_FILTER_CLIP(c, "preamp", -36.0, 36.0);
	...
	//CLASS_ATTR_FILTER_CLIP(c, "postamp", -110.0, 0);
	...
	CLASS_ATTR_FILTER_CLIP(c, "threshold", -110.0, 0.0);
	...
	CLASS_ATTR_FILTER_CLIP(c, "release", 1.0, 10000.0);
...
void *limisnap_new(t_symbol *s, long argc, t_atom *argv)
{
	...
	x->x_out = outlet_new((t_object *)x, NULL); // message outlet
	...
	x->x_gr = 1.0;
	x->x_meters = (t_atom*)sysmem_newptr(x->x_channelcount * sizeof(t_atom));
	t_atom* v;
	long i;
	for (v = x->x_meters, i = 0; i < x->x_channelcount; v++, i++)
	{
		atom_setfloat(v, 0.0);
	}
...
void limisnap_bang(t_limisnap *x)
{
	...
	t_atom argv_gr[1];
	auto meter_values = x->x_limiter->meters();
	long i;
	t_atom* v;
	for (v = x->x_meters, i = 0; i < x->x_channelcount; v++, i++) {
		atom_setfloat(v, meter_values[i]);
	}

	atom_setfloat(argv_gr, x->x_gr);
	outlet_anything(x->x_out, ps_gr, 1, argv_gr);

	outlet_anything(x->x_out, ps_meters, x->x_channelcount, x->x_meters);
}

t_max_err limisnap_setmetermode(t_limisnap* x, void* attr, long argc, t_atom* argv)
{
	x->x_meter_mode = atom_getlong(argv);
	x->x_limiter->metermode(x->x_meter_mode);
	return MAX_ERR_NONE;
}

t_max_err limisnap_setmeterdecay(t_limisnap* x, void* attr, long argc, t_atom* argv)
{
	x->x_meter_decay = atom_getfloat(argv);
	x->x_limiter->decaycoeff(x->x_meter_decay);
	return MAX_ERR_NONE;
}
